...
!BOC
   ...
   if (lake) then
      call fm%register('Af', 'm2', 'hypsograph at grid interfaces', standard_name='??', dimensions=(/id_dim_z/), data1d=Af(1:nlev), category='meanflow')
   end if
   ...
   call fm%register('water_balance', 'm3/s', 'water_balance', standard_name='??', data0d=net_water_balance, category='meanflow')
   if (lake) then
      call fm%register('int_water_balance', 'm3', 'integrated total water balance', data0d=int_water_balance, category='meanflow')
   else
