!$Id: turbulence_adv.F90,v 1.4.2.1 2005-07-06 09:25:05 hb Exp $
#include"cppdefs.h"
!-----------------------------------------------------------------------
!BOP
!
...
!  Revision 1.4.2.1  2005-07-06 09:25:05  hb
...
!BOC
   ...
   if (w_adv_method .ne. 0) then
      ...
      if ((tke_method.eq.tke_keps).or.(tke_method.eq.tke_MY)) &
         call w_split_it_adv_1d(N-1,dt,h_t,ho_t,tke,w_t,w_adv_discr, &
                             surf_flux,bott_flux,1)
      if (len_scale_method.eq.diss_eq)  & 
         call w_split_it_adv_1d(N-1,dt,h_t,ho_t,eps,w_t,w_adv_discr, &
                             surf_flux,bott_flux,1)
      if ((len_scale_method.eq.length_eq).or.(len_scale_method.eq.generic_eq)) & 
         call w_split_it_adv_1d(N-1,dt,h_t,ho_t,L,w_t,w_adv_discr, &
                             surf_flux,bott_flux,1)
   end if
   ...
   if (grid_method .ne. 0) then
      ...
      w_grid_t(N)=0.
      if ((tke_method.eq.tke_keps).or.(tke_method.eq.tke_MY)) &
         call w_split_it_adv_1d(N-1,dt,h_t,ho_t,tke,w_grid_t,w_adv_discr, &
                             surf_flux,bott_flux,2)
      if (len_scale_method.eq.diss_eq)  & 
         call w_split_it_adv_1d(N-1,dt,h_t,ho_t,eps,w_grid_t,w_adv_discr, &
                             surf_flux,bott_flux,2)
      if ((len_scale_method.eq.length_eq).or.(len_scale_method.eq.generic_eq)) & 
         call w_split_it_adv_1d(N-1,dt,h_t,ho_t,L,w_grid_t,w_adv_discr, &
                             surf_flux,bott_flux,2)
   end if
