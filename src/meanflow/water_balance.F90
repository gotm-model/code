...
!STDERR size(Af),nlev
      ...
      sumVc = sum(Vc(1:nlev))
      net_water_balance = sum(Qlayer(1:nlev)) + Af(nlev)*(evap + precip%value)
      if (zeta%method.eq.1 .or. zeta%method.eq.2) then
...
!KBSTDERR 'net ',net_water_balance
      select case (water_balance_method)
         ...
         case(WATER_BALANCE_ZETA)
            if (zeta%method .ne. 3) stop 'WATER_BALANCE_ZETA requires zeta_method=3'
