   ...
   if (present(field_manager)) then
      ...
      do i=1,size(model%bottom_state_variables)
         if (bottom_everywhere) then
            call register_field(field_manager, model%bottom_state_variables(i), data1d=cc(1:,size(model%state_variables)+i), dimensions=(/id_dim_z/))
         else
...
!BOC
   ...
   end subroutine check_fabm_expressions
!EOC
...
!BOC
   ...
   end subroutine update_fabm_expressions
!EOC
