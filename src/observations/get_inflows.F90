...
!BOC
   if (init_saved_vars) then
      ...
      if (rc /= 0) stop 'read_inflows: Error allocating memory (prof1)'
      prof1 = _ZERO_

      if (allocated(prof2)) deallocate(prof2)
      allocate(prof2(0:nlev,cols),stat=rc)
      if (rc /= 0) stop 'read_inflows: Error allocating memory (prof2)'
      prof2 = _ZERO_

      if (allocated(alpha)) deallocate(alpha)
      allocate(alpha(0:nlev,cols),stat=rc)
      if (rc /= 0) stop 'read_inflows: Error allocating memory (alpha)'
   end if
